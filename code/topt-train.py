...
def plot_prediction_results(y_true, y_pred, save_path="../pictures/topt/final_test_prediction_plot.png"):
    ...
    plt.xlabel('Experimental Topt (℃)')
    plt.ylabel('Predicted Topt (℃)')
    plt.legend()
    plt.tight_layout()
...
def load_resume_checkpoint(path, model, plm_model, optimizer, scheduler, scaler):
    if os.path.exists(path):
        ...
        print(f"Epoch {start_epoch + 1} continue")
        print(f"present best Validation RMSE: {best_dev_rmse:.4f}")
        print(f"present best Validation R2:   {best_dev_r2:.4f}\n")
...
def run_training():
    ...
    print("=" * 60 + "\n")
    out_df = df_te.copy()
    out_df['true_topt'] = y_true
    out_df['pred_topt'] = y_pred
