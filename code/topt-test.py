...
class MultimodalToptPredictor:
    ...
    def paper_style_benchmark_visualization(self, df, save_path, task_col='topt'):
        if task_col not in df.columns:
            return
        y_true = df[task_col].values
        y_pred = df['pred_topt'].values
        ...
        plt.xlabel('Experimental $T_{opt}$ (°C)', fontsize=12, fontweight='bold')
        plt.ylabel('Predicted $T_{opt}$ (°C)', fontsize=12, fontweight='bold')
...
def main():
    ...
    parser.add_argument('--output', default='/home/ys/new/TGC-Net/data/result/topt/result1.csv')
    ...
    parser.add_argument('--quality_path', default='/home/ys/new/TGC-Net/data/topt/test/291-wild-type_quality.csv')
